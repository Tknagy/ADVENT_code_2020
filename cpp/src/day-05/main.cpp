...
unsigned int seat_id(const std::string &line)
{
    ...
    for (size_t i = 0; i < 7; i++) {
        if (line[i] == 'F') max -= (max-min) / 2.0;
        else if (line[i] == 'B') min += (max-min) / 2.0 + 1;
        else std::abort();
    }
    if (min != max) std::abort();
    unsigned int row = min;

    min = 0;
    max = 7;
    for (size_t i = 0; i < 3; i++) {
        if (line[7 + i] == 'L') max -= (max-min) / 2.0;
        else if (line[7 + i] == 'R') min += (max-min) / 2.0 + 1;
        else std::abort();
    }
    if (min != max) std::abort();
...
int main(int argc, char const *argv[])
{
    ...
    const auto it = std::adjacent_find(ids.begin(), ids.end(), [](unsigned int a, unsigned int b) {
        return (b-a) != 1;
    });
